...
Usage: node remove-debug-safe.js [options]
...
Example:
  node remove-debug-safe.js --dry-run    # Preview changes
  node remove-debug-safe.js              # Remove debug code
`);
  process.exit(0);
}
...
// Configuration
const config = {
  // Only process Dart files in lib directory (excluding functions)
  includePaths: [
    'lib/**/*.dart'
  ],
  ...
  debugPatterns: [
    ...
    },
    {
      ...
      pattern: /^\s*print\(['"](=== |>>> |<<< |\[DEBUG\]|\[INFO\]|\[WARN\]|\[ERROR\]).*?['"]\);\s*$/gm,
      replacement: '',
  ...
  backupDir: `debug-backup-${new Date().toISOString().slice(0,10).replace(/-/g,'')}`
};
...
// Main function
function main() {
  console.log('🔍 Safe Debug Code Removal Script');
  console.log('=================================\n');
  ...
  console.log(`📁 Found ${files.length} Dart files in lib/ to process\n`);
  ...
  if (isDryRun && stats.filesModified > 0) {
    ...
    console.log('   node remove-debug-safe.js');
  }
  
  console.log('\n✅ ' + (isDryRun ? 'Dry run' : 'Debug removal') + ' complete!');
}
