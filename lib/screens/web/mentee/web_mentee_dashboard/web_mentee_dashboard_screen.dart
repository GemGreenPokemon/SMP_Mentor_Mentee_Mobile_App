...
class _WebMenteeDashboardScreenState extends State<WebMenteeDashboardScreen> 
  ...
  Future<void> _clearMeeting(String meetingId) async {
    print('🔍 Mentee _clearMeeting: Starting to clear meeting $meetingId');
    try {
      // Show loading indicator
      showDialog(
        context: context,
      ...
      print('🔍 Mentee _clearMeeting: Calling hideMeeting...');
      // Hide the meeting from view
      final success = await _meetingService.hideMeeting(meetingId);
      print('🔍 Mentee _clearMeeting: hideMeeting returned: $success');
      
      // Close loading dialog
      if (mounted) Navigator.pop(context);

      if (success) {
        print('🔍 Mentee _clearMeeting: Success! Showing success message');
        // Show success message
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        print('🔍 Mentee _clearMeeting: Refreshing dashboard data...');
        // Refresh dashboard data
        await _refreshController.refresh();
        print('🔍 Mentee _clearMeeting: Dashboard refresh complete');
      } else {
        print('🔍 Mentee _clearMeeting: hideMeeting returned false');
        throw Exception('Failed to clear meeting');
      }
    } catch (e) {
      print('🔍 Mentee _clearMeeting: Error occurred: $e');
      print('🔍 Mentee _clearMeeting: Error type: ${e.runtimeType}');
      // Close loading dialog if still open
      if (mounted && Navigator.canPop(context)) Navigator.pop(context);
